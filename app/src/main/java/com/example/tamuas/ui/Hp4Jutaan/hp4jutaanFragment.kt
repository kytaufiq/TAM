package com.example.tamuas.ui.Hp4Jutaan
...
class hp4jutaanFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        Log.d("Hp4JutaanFragment", "onCreateView called")
        return inflater.inflate(R.layout.fragment_hp4jutaan, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        Log.d("Hp4JutaanFragment", "onViewCreated called")

        try {
            loadHp4JutaanSmartphones(view)
        } catch (e: Exception) {
            Log.e("Hp4JutaanFragment", "Error in onViewCreated", e)
        }
    }

    private fun loadHp4JutaanSmartphones(view: View) {
        Log.d("Hp4JutaanFragment", "Starting to load 4 jutaan smartphones")

        lifecycleScope.launch {
            try {
                val hp4jutaanSmartphones = smartphoneRepository.getPrice4MSmartphones()

                Log.d("Hp4JutaanFragment", "Got ${hp4jutaanSmartphones.size} 4 jutaan smartphones")

                if (hp4jutaanSmartphones.isNotEmpty()) {
                    // Populate champion card (first smartphone)
                    populateChampionCard(view, hp4jutaanSmartphones[0])

                    // Populate ranking cards (all smartphones)
                    populateRankingCards(view, hp4jutaanSmartphones)

                    Log.d("Hp4JutaanFragment", "Successfully populated 4 jutaan cards")
                } else {
                    Log.w("Hp4JutaanFragment", "No 4 jutaan smartphones found")
                    showNoDataMessage(view)
                }

            } catch (e: Exception) {
                Log.e("Hp4JutaanFragment", "Error loading 4 jutaan smartphones", e)
    ...
    private fun showNoDataMessage(view: View) {
        try {
            val championContainer = view.findViewById<LinearLayout>(R.id.hp4jutaan_champion_container)
            championContainer?.removeAllViews()

            val noDataText = TextView(requireContext()).apply {
                text = "Tidak ada data HP 4 jutaan"
                textSize = 16f
                setPadding(32, 32, 32, 32)
            }
            championContainer?.addView(noDataText)
        } catch (e: Exception) {
            Log.e("Hp4JutaanFragment", "Error showing no data message", e)
        }
    }
    ...
    private fun showErrorMessage(view: View) {
        try {
            ...
            val errorText = TextView(requireContext()).apply {
                text = "Error loading HP 4 jutaan"
                textSize = 16f
                setPadding(32, 32, 32, 32)
            }
            championContainer?.addView(errorText)
        } catch (e: Exception) {
            Log.e("Hp4JutaanFragment", "Error showing error message", e)
        }
    }
    ...
    private fun populateRankingCards(view: View, smartphones: List<com.example.tamuas.models.Smartphone>) {
        val rankingContainer = view.findViewById<LinearLayout>(R.id.hp4jutaan_ranking_container)
    ...
    private fun createChampionCardProgrammatically(smartphone: com.example.tamuas.models.Smartphone): View {
        ...
        // Premium Mid-Range Crown header
    ...
    private fun createRankingCardProgrammatically(smartphone: com.example.tamuas.models.Smartphone, rank: Int): View {
        ...
        // Premium Mid-Range Specs Layout
    ...
    private fun navigateToDetail(smartphoneId: String) {
        try {
            Log.d("Hp4jutaanFragment", "Navigating to detail for smartphone: $smartphoneId")

            if (!isAdded || context == null) {
                Log.w("Hp4jutaanFragment", "Fragment not attached")
                return
            }
            ...
            navController.navigate(R.id.action_hp4jutaan_to_detail, bundle)

            Log.d("Hp4jutaanFragment", "Navigation to detail initiated")

        } catch (e: Exception) {
            Log.e("Hp4jutaanFragment", "Error navigating to detail", e)
            if (isAdded && context != null) {
                Toast.makeText(
                    requireContext(),
