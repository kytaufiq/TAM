package com.example.tamuas.ui.vivo
...
class VivoFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<TextView>(R.id.btn_back)?.setOnClickListener {
            try {
                ...
                Log.e("VivoFragment", "Error navigating back with NavController", e)
                try {
                    parentFragmentManager.popBackStack()
                } catch (fallbackError: Exception) {
                    Log.e("VivoFragment", "Fallback navigation also failed", fallbackError)
        ...
        loadVivoSmartphones(view)
    }

    private fun loadVivoSmartphones(view: View) {
        Log.d("VivoFragment", "Starting to load Vivo smartphones")

        if (!isAdded || context == null) {
            Log.w("VivoFragment", "Fragment not attached, skipping data load")
            return
        }

        viewLifecycleOwner.lifecycleScope.launch {
            try {
                val vivoSmartphones = smartphoneRepository.getVivoSmartphones()

                if (!isAdded || context == null) {
                    Log.w("VivoFragment", "Fragment detached during data load")
                    return@launch
                }

                Log.d("VivoFragment", "Got ${vivoSmartphones.size} Vivo smartphones")

                if (vivoSmartphones.isNotEmpty()) {
                    populateChampionCard(view, vivoSmartphones[0])
                    populateRankingCards(view, vivoSmartphones)
                    Log.d("VivoFragment", "Successfully populated Vivo cards")
                } else {
                    Log.w("VivoFragment", "No Vivo smartphones found")
                    showNoDataMessage(view)
                }

            } catch (e: Exception) {
                Log.e("VivoFragment", "Error loading Vivo smartphones", e)
                if (isAdded && context != null) {
    ...
    private fun showNoDataMessage(view: View) {
        try {
            ...
            val championContainer = view.findViewById<LinearLayout>(R.id.vivo_champion_container)
            championContainer?.removeAllViews()

            val noDataText = TextView(requireContext()).apply {
                text = "Tidak ada data Vivo smartphones"
                textSize = 16f
                setPadding(32, 32, 32, 32)
            }
            championContainer?.addView(noDataText)
        } catch (e: Exception) {
            Log.e("VivoFragment", "Error showing no data message", e)
        }
    }
    ...
    private fun showErrorMessage(view: View) {
        try {
            ...
            val errorText = TextView(requireContext()).apply {
                text = "Error loading Vivo smartphones"
                textSize = 16f
                setPadding(32, 32, 32, 32)
            }
            championContainer?.addView(errorText)
        } catch (e: Exception) {
            Log.e("VivoFragment", "Error showing error message", e)
        }
    }
    ...
    private fun populateChampionCard(view: View, champion: com.example.tamuas.models.Smartphone) {
        try {
            ...
            Log.e("VivoFragment", "Error populating champion card", e)
        }
    }
    ...
    private fun populateRankingCards(view: View, smartphones: List<com.example.tamuas.models.Smartphone>) {
        try {
            ...
            val rankingContainer = view.findViewById<LinearLayout>(R.id.vivo_ranking_container)
            ...
            Log.e("VivoFragment", "Error populating ranking cards", e)
        }
    }
    ...
    private fun createChampionCardProgrammatically(smartphone: com.example.tamuas.models.Smartphone): View {
        ...
        // Main card layout dengan Vivo theme
        val cardLayout = LinearLayout(context).apply {
            ...
            setPadding(32, 32, 32, 32)
            setBackgroundColor(android.graphics.Color.parseColor("#E8EAF6")) // Vivo purple theme
            elevation = 8f
        }
        ...
        val crownText = TextView(context).apply {
            text = "🏆 Terbaik dari Vivo"
            textSize = 16f
            setTextColor(android.graphics.Color.parseColor("#5E35B1"))
        ...
        try {
            ...
            Log.e("VivoFragment", "Error loading image with Glide", e)
        ...
        val bestChoiceView = TextView(context).apply {
            ...
            setBackgroundColor(android.graphics.Color.parseColor("#673AB7")) // Vivo purple
        }
    ...
    private fun createRankingCardFromXML(smartphone: com.example.tamuas.models.Smartphone, rank: Int): View {
        ...
        val cardView = inflater.inflate(R.layout.item_smartphone_vivo, null)

        try {
            // Set rank
            val rankNumber = cardView.findViewById<TextView>(R.id.tv_vivo_rank_number)
            ...
            // Set image
            val imageView = cardView.findViewById<ImageView>(R.id.iv_vivo_image)
            try {
                if (isAdded && context != null) {
                    Glide.with(this)
                        .load(smartphone.imageUrl)
                ...
                Log.e("VivoFragment", "Error loading ranking image", e)
                imageView.setImageResource(R.drawable.hp_iphone15pro)
            }

            // Set name
            val nameView = cardView.findViewById<TextView>(R.id.tv_vivo_name)
            nameView.text = smartphone.name

            // Set brand
            val brandView = cardView.findViewById<TextView>(R.id.tv_vivo_brand)
            brandView.text = smartphone.brand

            // Set rating
            val ratingView = cardView.findViewById<TextView>(R.id.tv_vivo_rating)
            ratingView.text = "⭐ ${smartphone.rating}"

            // Set processor
            val processorView = cardView.findViewById<TextView>(R.id.tv_vivo_processor)
            processorView.text = smartphone.processor

            // Set storage
            val storageView = cardView.findViewById<TextView>(R.id.tv_vivo_storage)
            storageView.text = smartphone.storage.split("/")[0] // Take first storage option

            // Set camera
            val cameraView = cardView.findViewById<TextView>(R.id.tv_vivo_camera)
            val cameraText = extractMainCamera(smartphone.camera)
            cameraView.text = cameraText

            // Set display
            val displayView = cardView.findViewById<TextView>(R.id.tv_vivo_display)
            val displayText = extractDisplaySize(smartphone.display)
            displayView.text = displayText

            // Set description
            val descriptionView = cardView.findViewById<TextView>(R.id.tv_vivo_description)
            descriptionView.text = smartphone.description

            // Set price
            val priceView = cardView.findViewById<TextView>(R.id.tv_vivo_price)
            val formatter = NumberFormat.getInstance(Locale("id", "ID"))
            val formattedPrice = "Rp ${formatter.format(smartphone.price)}"
            priceView.text = formattedPrice

            // Set detail button click
            val detailButton = cardView.findViewById<TextView>(R.id.btn_vivo_detail)
            detailButton.setOnClickListener {
                navigateToDetail(smartphone.id)
            }

        } catch (e: Exception) {
            Log.e("VivoFragment", "Error setting up ranking card", e)
        }
    ...
    private fun extractMainCamera(cameraString: String): String {
        return when {
            cameraString.contains("50MP Zeiss") -> "50MP Zeiss"
            cameraString.contains("50MP OIS") -> "50MP OIS"
    ...
    private fun navigateToDetail(smartphoneId: String) {
        try {
            ...
            Log.d("VivoFragment", "Navigating to detail for smartphone: $smartphoneId")
            ...
            navController.navigate(R.id.action_vivo_to_detail, bundle)

            Log.d("VivoFragment", "Navigation to detail initiated from Vivo")

        } catch (e: Exception) {
            Log.e("VivoFragment", "Error navigating to detail", e)
        }
    }
}