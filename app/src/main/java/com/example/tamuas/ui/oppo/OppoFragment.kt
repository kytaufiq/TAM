package com.example.tamuas.ui.oppo
...
class OppoFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<TextView>(R.id.btn_back)?.setOnClickListener {
            try {
                ...
                Log.e("OppoFragment", "Error navigating back with NavController", e)
                try {
                    parentFragmentManager.popBackStack()
                } catch (fallbackError: Exception) {
                    Log.e("OppoFragment", "Fallback navigation also failed", fallbackError)
        ...
        loadOppoSmartphones(view)
    }

    private fun loadOppoSmartphones(view: View) {
        Log.d("OppoFragment", "Starting to load Oppo smartphones")

        if (!isAdded || context == null) {
            Log.w("OppoFragment", "Fragment not attached, skipping data load")
            return
        }

        viewLifecycleOwner.lifecycleScope.launch {
            try {
                val oppoSmartphones = smartphoneRepository.getOppoSmartphones()

                if (!isAdded || context == null) {
                    Log.w("OppoFragment", "Fragment detached during data load")
                    return@launch
                }

                Log.d("OppoFragment", "Got ${oppoSmartphones.size} Oppo smartphones")

                if (oppoSmartphones.isNotEmpty()) {
                    populateChampionCard(view, oppoSmartphones[0])
                    populateRankingCards(view, oppoSmartphones)
                    Log.d("OppoFragment", "Successfully populated Oppo cards")
                } else {
                    Log.w("OppoFragment", "No Oppo smartphones found")
                    showNoDataMessage(view)
                }

            } catch (e: Exception) {
                Log.e("OppoFragment", "Error loading Oppo smartphones", e)
                if (isAdded && context != null) {
    ...
    private fun showNoDataMessage(view: View) {
        try {
            ...
            val championContainer = view.findViewById<LinearLayout>(R.id.oppo_champion_container)
            championContainer?.removeAllViews()

            val noDataText = TextView(requireContext()).apply {
                text = "Tidak ada data Oppo smartphones"
                textSize = 16f
                setPadding(32, 32, 32, 32)
            }
            championContainer?.addView(noDataText)
        } catch (e: Exception) {
            Log.e("OppoFragment", "Error showing no data message", e)
        }
    }
    ...
    private fun showErrorMessage(view: View) {
        try {
            ...
            val errorText = TextView(requireContext()).apply {
                text = "Error loading Oppo smartphones"
                textSize = 16f
                setPadding(32, 32, 32, 32)
            }
            championContainer?.addView(errorText)
        } catch (e: Exception) {
            Log.e("OppoFragment", "Error showing error message", e)
        }
    }
    ...
    private fun populateChampionCard(view: View, champion: com.example.tamuas.models.Smartphone) {
        try {
            ...
            Log.e("OppoFragment", "Error populating champion card", e)
        }
    }
    ...
    private fun populateRankingCards(view: View, smartphones: List<com.example.tamuas.models.Smartphone>) {
        try {
            ...
            val rankingContainer = view.findViewById<LinearLayout>(R.id.oppo_ranking_container)
            ...
            Log.e("OppoFragment", "Error populating ranking cards", e)
        }
    }
    ...
    private fun createChampionCardProgrammatically(smartphone: com.example.tamuas.models.Smartphone): View {
        ...
        // Main card layout dengan Oppo theme
        val cardLayout = LinearLayout(context).apply {
            ...
            setPadding(32, 32, 32, 32)
            setBackgroundColor(android.graphics.Color.parseColor("#E8F5E8")) // Oppo green theme
            elevation = 8f
        }
        ...
        try {
            ...
            Log.e("OppoFragment", "Error loading image with Glide", e)
        ...
        val bestChoiceView = TextView(context).apply {
            ...
            setBackgroundColor(android.graphics.Color.parseColor("#4CAF50")) // Oppo green
        }
    ...
    private fun createRankingCardFromXML(smartphone: com.example.tamuas.models.Smartphone, rank: Int): View {
        ...
        val cardView = inflater.inflate(R.layout.item_smartphone_oppo, null)

        try {
            // Set rank
            val rankNumber = cardView.findViewById<TextView>(R.id.tv_oppo_rank_number)
            ...
            // Set image
            val imageView = cardView.findViewById<ImageView>(R.id.iv_oppo_image)
            try {
                if (isAdded && context != null) {
                    Glide.with(this)
                        .load(smartphone.imageUrl)
                ...
                Log.e("OppoFragment", "Error loading ranking image", e)
                imageView.setImageResource(R.drawable.hp_iphone15pro)
            }

            // Set name
            val nameView = cardView.findViewById<TextView>(R.id.tv_oppo_name)
            nameView.text = smartphone.name

            // Set brand
            val brandView = cardView.findViewById<TextView>(R.id.tv_oppo_brand)
            brandView.text = smartphone.brand

            // Set rating
            val ratingView = cardView.findViewById<TextView>(R.id.tv_oppo_rating)
            ratingView.text = "⭐ ${smartphone.rating}"

            // Set processor
            val processorView = cardView.findViewById<TextView>(R.id.tv_oppo_processor)
            processorView.text = smartphone.processor

            // Set storage
            val storageView = cardView.findViewById<TextView>(R.id.tv_oppo_storage)
            storageView.text = smartphone.storage.split("/")[0] // Take first storage option

            // Set camera
            val cameraView = cardView.findViewById<TextView>(R.id.tv_oppo_camera)
            val cameraText = extractMainCamera(smartphone.camera)
            cameraView.text = cameraText

            // Set display
            val displayView = cardView.findViewById<TextView>(R.id.tv_oppo_display)
            val displayText = extractDisplaySize(smartphone.display)
            displayView.text = displayText

            // Set description
            val descriptionView = cardView.findViewById<TextView>(R.id.tv_oppo_description)
            descriptionView.text = smartphone.description

            // Set price
            val priceView = cardView.findViewById<TextView>(R.id.tv_oppo_price)
            val formatter = NumberFormat.getInstance(Locale("id", "ID"))
            val formattedPrice = "Rp ${formatter.format(smartphone.price)}"
            priceView.text = formattedPrice

            // Set detail button click
            val detailButton = cardView.findViewById<TextView>(R.id.btn_oppo_detail)
            detailButton.setOnClickListener {
                navigateToDetail(smartphone.id)
            }

        } catch (e: Exception) {
            Log.e("OppoFragment", "Error setting up ranking card", e)
        }
    ...
    private fun extractMainCamera(cameraString: String): String {
        return when {
            cameraString.contains("50MP Hasselblad") -> "50MP Hasselblad"
            cameraString.contains("50MP Portrait") -> "50MP Portrait"
    ...
    private fun navigateToDetail(smartphoneId: String) {
        try {
            ...
            Log.d("OppoFragment", "Navigating to detail for smartphone: $smartphoneId")
            ...
            navController.navigate(R.id.action_oppo_to_detail, bundle)

            Log.d("OppoFragment", "Navigation to detail initiated from Oppo")

        } catch (e: Exception) {
            Log.e("OppoFragment", "Error navigating to detail", e)
        }
    }
}