package com.example.tamuas.ui.xiaomi
...
class XiaomiFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<TextView>(R.id.btn_back)?.setOnClickListener {
            try {
                ...
                Log.e("XiaomiFragment", "Error navigating back with NavController", e)
                try {
                    parentFragmentManager.popBackStack()
                } catch (fallbackError: Exception) {
                    Log.e("XiaomiFragment", "Fallback navigation also failed", fallbackError)
        ...
        loadXiaomiSmartphones(view)
    }

    private fun loadXiaomiSmartphones(view: View) {
        Log.d("XiaomiFragment", "Starting to load Xiaomi smartphones")

        if (!isAdded || context == null) {
            Log.w("XiaomiFragment", "Fragment not attached, skipping data load")
            return
        }

        viewLifecycleOwner.lifecycleScope.launch {
            try {
                val xiaomiSmartphones = smartphoneRepository.getXiaomiSmartphones()

                if (!isAdded || context == null) {
                    Log.w("XiaomiFragment", "Fragment detached during data load")
                    return@launch
                }

                Log.d("XiaomiFragment", "Got ${xiaomiSmartphones.size} Xiaomi smartphones")

                if (xiaomiSmartphones.isNotEmpty()) {
                    populateChampionCard(view, xiaomiSmartphones[0])
                    populateRankingCards(view, xiaomiSmartphones)
                    Log.d("XiaomiFragment", "Successfully populated Xiaomi cards")
                } else {
                    Log.w("XiaomiFragment", "No Xiaomi smartphones found")
                    showNoDataMessage(view)
                }

            } catch (e: Exception) {
                Log.e("XiaomiFragment", "Error loading Xiaomi smartphones", e)
                if (isAdded && context != null) {
    ...
    private fun showNoDataMessage(view: View) {
        try {
            ...
            val championContainer = view.findViewById<LinearLayout>(R.id.xiaomi_champion_container)
            championContainer?.removeAllViews()

            val noDataText = TextView(requireContext()).apply {
                text = "Tidak ada data Xiaomi smartphones"
                textSize = 16f
                setPadding(32, 32, 32, 32)
            }
            championContainer?.addView(noDataText)
        } catch (e: Exception) {
            Log.e("XiaomiFragment", "Error showing no data message", e)
        }
    }
    ...
    private fun showErrorMessage(view: View) {
        try {
            ...
            val errorText = TextView(requireContext()).apply {
                text = "Error loading Xiaomi smartphones"
                textSize = 16f
                setPadding(32, 32, 32, 32)
            }
            championContainer?.addView(errorText)
        } catch (e: Exception) {
            Log.e("XiaomiFragment", "Error showing error message", e)
        }
    }
    ...
    private fun populateChampionCard(view: View, champion: com.example.tamuas.models.Smartphone) {
        try {
            ...
            Log.e("XiaomiFragment", "Error populating champion card", e)
        }
    }
    ...
    private fun populateRankingCards(view: View, smartphones: List<com.example.tamuas.models.Smartphone>) {
        try {
            ...
            val rankingContainer = view.findViewById<LinearLayout>(R.id.xiaomi_ranking_container)
            ...
            Log.e("XiaomiFragment", "Error populating ranking cards", e)
        }
    }
    ...
    private fun createChampionCardProgrammatically(smartphone: com.example.tamuas.models.Smartphone): View {
        ...
        // Main card layout dengan Xiaomi theme
        val cardLayout = LinearLayout(context).apply {
            ...
            setPadding(32, 32, 32, 32)
            setBackgroundColor(android.graphics.Color.parseColor("#FFF3E0")) // Xiaomi orange theme
            elevation = 8f
        }
        ...
        try {
            ...
            Log.e("XiaomiFragment", "Error loading image with Glide", e)
        ...
        val bestChoiceView = TextView(context).apply {
            ...
            setBackgroundColor(android.graphics.Color.parseColor("#FF8F00")) // Xiaomi orange
        }
        ...
        val priceView = TextView(context).apply {
            text = formattedPrice
            textSize = 16f
            setTextColor(android.graphics.Color.parseColor("#FF8F00"))
            setTypeface(null, android.graphics.Typeface.BOLD)
        }
    ...
    private fun createRankingCardFromXML(smartphone: com.example.tamuas.models.Smartphone, rank: Int): View {
        ...
        val cardView = inflater.inflate(R.layout.item_smartphone_xiaomi, null)

        try {
            // Set rank
            val rankNumber = cardView.findViewById<TextView>(R.id.tv_xiaomi_rank_number)
            ...
            // Set image
            val imageView = cardView.findViewById<ImageView>(R.id.iv_xiaomi_image)
            try {
                if (isAdded && context != null) {
                    Glide.with(this)
                        .load(smartphone.imageUrl)
                ...
                Log.e("XiaomiFragment", "Error loading ranking image", e)
                imageView.setImageResource(R.drawable.hp_iphone15pro)
            }

            // Set name
            val nameView = cardView.findViewById<TextView>(R.id.tv_xiaomi_name)
            nameView.text = smartphone.name

            // Set brand
            val brandView = cardView.findViewById<TextView>(R.id.tv_xiaomi_brand)
            brandView.text = smartphone.brand

            // Set rating
            val ratingView = cardView.findViewById<TextView>(R.id.tv_xiaomi_rating)
            ratingView.text = "⭐ ${smartphone.rating}"

            // Set processor
            val processorView = cardView.findViewById<TextView>(R.id.tv_xiaomi_processor)
            processorView.text = smartphone.processor

            // Set storage
            val storageView = cardView.findViewById<TextView>(R.id.tv_xiaomi_storage)
            storageView.text = smartphone.storage.split("/")[0] // Take first storage option

            // Set camera
            val cameraView = cardView.findViewById<TextView>(R.id.tv_xiaomi_camera)
            val cameraText = extractMainCamera(smartphone.camera)
            cameraView.text = cameraText

            // Set display
            val displayView = cardView.findViewById<TextView>(R.id.tv_xiaomi_display)
            val displayText = extractDisplaySize(smartphone.display)
            displayView.text = displayText

            // Set description
            val descriptionView = cardView.findViewById<TextView>(R.id.tv_xiaomi_description)
            descriptionView.text = smartphone.description

            // Set price
            val priceView = cardView.findViewById<TextView>(R.id.tv_xiaomi_price)
            val formatter = NumberFormat.getInstance(Locale("id", "ID"))
            val formattedPrice = "Rp ${formatter.format(smartphone.price)}"
            priceView.text = formattedPrice

            // Set detail button click
            val detailButton = cardView.findViewById<TextView>(R.id.btn_xiaomi_detail)
            detailButton.setOnClickListener {
                navigateToDetail(smartphone.id)
            }

        } catch (e: Exception) {
            Log.e("XiaomiFragment", "Error setting up ranking card", e)
        }
    ...
    private fun extractMainCamera(cameraString: String): String {
        return when {
            ...
            cameraString.contains("50MP") -> "50MP Leica"
            cameraString.contains("64MP") -> "64MP"
            else -> "50MP"
        }
    }
    ...
    private fun navigateToDetail(smartphoneId: String) {
        try {
            ...
            Log.d("XiaomiFragment", "Navigating to detail for smartphone: $smartphoneId")
            ...
            navController.navigate(R.id.action_xiaomi_to_detail, bundle)

            Log.d("XiaomiFragment", "Navigation to detail initiated from Xiaomi")

        } catch (e: Exception) {
            Log.e("XiaomiFragment", "Error navigating to detail", e)
        }
    }
}