package com.example.tamuas.ui.gaming
...
// Update class name dari gamingFragment ke GamingFragment (huruf besar)
class gamingFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        Log.d("GamingFragment", "onCreateView called")
        return inflater.inflate(R.layout.fragment_gaming, container, false)
    }

    // Update onViewCreated method dengan error handling:
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        Log.d("GamingFragment", "onViewCreated called")

        try {
            loadGamingSmartphones(view)
        } catch (e: Exception) {
            Log.e("GamingFragment", "Error in onViewCreated", e)
        }
    }

    // Update loadGamingSmartphones method:
    private fun loadGamingSmartphones(view: View) {
        Log.d("GamingFragment", "Starting to load gaming smartphones")

        lifecycleScope.launch {
            try {
                val gamingSmartphones = smartphoneRepository.getGamingSmartphones()

                Log.d("GamingFragment", "Got ${gamingSmartphones.size} gaming smartphones")

                if (gamingSmartphones.isNotEmpty()) {
                    // Populate champion card (first smartphone)
                    populateChampionCard(view, gamingSmartphones[0])

                    // Populate ranking cards (all smartphones)
                    populateRankingCards(view, gamingSmartphones)

                    Log.d("GamingFragment", "Successfully populated gaming cards")
                } else {
                    Log.w("GamingFragment", "No gaming smartphones found")
                    showNoDataMessage(view)
                }

            } catch (e: Exception) {
                Log.e("GamingFragment", "Error loading gaming smartphones", e)
    ...
    private fun showNoDataMessage(view: View) {
        try {
            val championContainer = view.findViewById<LinearLayout>(R.id.gaming_champion_container)
            championContainer?.removeAllViews()

            val noDataText = TextView(requireContext()).apply {
                text = "Tidak ada data gaming smartphone"
                textSize = 16f
                setPadding(32, 32, 32, 32)
            }
            championContainer?.addView(noDataText)
        } catch (e: Exception) {
            Log.e("GamingFragment", "Error showing no data message", e)
        }
    }
    ...
    private fun showErrorMessage(view: View) {
        try {
            ...
            val errorText = TextView(requireContext()).apply {
                text = "Error loading gaming smartphones"
                textSize = 16f
                setPadding(32, 32, 32, 32)
            }
            championContainer?.addView(errorText)
        } catch (e: Exception) {
            Log.e("GamingFragment", "Error showing error message", e)
        }
    }
    ...
    private fun populateRankingCards(view: View, smartphones: List<com.example.tamuas.models.Smartphone>) {
        val rankingContainer = view.findViewById<LinearLayout>(R.id.gaming_ranking_container)
    ...
    private fun createRankingCardProgrammatically(smartphone: com.example.tamuas.models.Smartphone, rank: Int): View {
        ...
        // Display and Battery Layout
        val specsLayout2 = LinearLayout(context).apply {
            orientation = LinearLayout.HORIZONTAL
        }

        val displayView = TextView(context).apply {
            text = if (smartphone.display.contains("Hz")) {
                smartphone.display.substringAfter("inch").trim()
            } else "120Hz"
            textSize = 11f
            setTextColor(android.graphics.Color.parseColor("#444444"))
            layoutParams = LinearLayout.LayoutParams(
        ...
        specsLayout2.addView(displayView)
    ...
    private fun navigateToDetail(smartphoneId: String) {
        try {
            Log.d("gamingFragment", "Navigating to detail for smartphone: $smartphoneId")

            if (!isAdded || context == null) {
                Log.w("gamingFragment", "Fragment not attached")
                return
            }
            ...
            navController.navigate(R.id.action_gaming_to_detail, bundle)

            Log.d("RekomendasiFragment", "Navigation to detail initiated")

        } catch (e: Exception) {
            Log.e("gamingFragment", "Error navigating to detail", e)
            if (isAdded && context != null) {
                Toast.makeText(
                    requireContext(),
