package com.example.tamuas.ui.apple
...
class AppleFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<TextView>(R.id.btn_back)?.setOnClickListener {
            try {
                ...
                Log.e("AppleFragment", "Error navigating back with NavController", e)
                // Fallback to fragment manager
                try {
                    parentFragmentManager.popBackStack()
                } catch (fallbackError: Exception) {
                    Log.e("AppleFragment", "Fallback navigation also failed", fallbackError)
        ...
        loadAppleSmartphones(view)
    }

    private fun loadAppleSmartphones(view: View) {
        Log.d("AppleFragment", "Starting to load Apple smartphones")

        // Tambahkan null check untuk context dan fragment state
        if (!isAdded || context == null) {
            Log.w("AppleFragment", "Fragment not attached, skipping data load")
            return
        }

        viewLifecycleOwner.lifecycleScope.launch {
            try {
                val appleSmartphones = smartphoneRepository.getAppleSmartphones()

                // Check jika fragment masih attached setelah async operation
                if (!isAdded || context == null) {
                    Log.w("AppleFragment", "Fragment detached during data load")
                    return@launch
                }

                Log.d("AppleFragment", "Got ${appleSmartphones.size} Apple smartphones")

                if (appleSmartphones.isNotEmpty()) {
                    // Populate champion card (first smartphone)
                    populateChampionCard(view, appleSmartphones[0])

                    // Populate ranking cards (all smartphones)
                    populateRankingCards(view, appleSmartphones)

                    Log.d("AppleFragment", "Successfully populated Apple cards")
                } else {
                    Log.w("AppleFragment", "No Apple smartphones found")
                    showNoDataMessage(view)
                }

            } catch (e: Exception) {
                Log.e("AppleFragment", "Error loading Apple smartphones", e)
                if (isAdded && context != null) {
    ...
    private fun showNoDataMessage(view: View) {
        try {
            ...
            val championContainer = view.findViewById<LinearLayout>(R.id.apple_champion_container)
            championContainer?.removeAllViews()

            val noDataText = TextView(requireContext()).apply {
                text = "Tidak ada data Apple smartphones"
                textSize = 16f
                setPadding(32, 32, 32, 32)
            }
            championContainer?.addView(noDataText)
        } catch (e: Exception) {
            Log.e("AppleFragment", "Error showing no data message", e)
        }
    }
    ...
    private fun showErrorMessage(view: View) {
        try {
            ...
            val errorText = TextView(requireContext()).apply {
                text = "Error loading Apple smartphones"
                textSize = 16f
                setPadding(32, 32, 32, 32)
            }
            championContainer?.addView(errorText)
        } catch (e: Exception) {
            Log.e("AppleFragment", "Error showing error message", e)
        }
    }
    ...
    private fun populateChampionCard(view: View, champion: com.example.tamuas.models.Smartphone) {
        try {
            ...
            Log.e("AppleFragment", "Error populating champion card", e)
        }
    }
    ...
    private fun populateRankingCards(view: View, smartphones: List<com.example.tamuas.models.Smartphone>) {
        try {
            ...
            val rankingContainer = view.findViewById<LinearLayout>(R.id.apple_ranking_container)

            if (rankingContainer != null) {
                // Clear existing views
                rankingContainer.removeAllViews()

                // Create ranking cards using XML layout
            ...
            Log.e("AppleFragment", "Error populating ranking cards", e)
        }
    }
    ...
    private fun createChampionCardProgrammatically(smartphone: com.example.tamuas.models.Smartphone): View {
        ...
        try {
            ...
            Log.e("AppleFragment", "Error loading image with Glide", e)
    ...
    private fun createRankingCardFromXML(smartphone: com.example.tamuas.models.Smartphone, rank: Int): View {
        ...
        val cardView = inflater.inflate(R.layout.item_smartphone_apple, null)

        try {
            // Set rank
            val rankNumber = cardView.findViewById<TextView>(R.id.tv_apple_rank_number)
            ...
            // Set image
            val imageView = cardView.findViewById<ImageView>(R.id.iv_apple_image)
            try {
                if (isAdded && context != null) {
                    Glide.with(this)
                        .load(smartphone.imageUrl)
                ...
                Log.e("AppleFragment", "Error loading ranking image", e)
                imageView.setImageResource(R.drawable.hp_iphone15pro)
            }

            // Set name
            val nameView = cardView.findViewById<TextView>(R.id.tv_apple_name)
            nameView.text = smartphone.name

            // Set brand
            val brandView = cardView.findViewById<TextView>(R.id.tv_apple_brand)
            brandView.text = smartphone.brand

            // Set rating
            val ratingView = cardView.findViewById<TextView>(R.id.tv_apple_rating)
            ratingView.text = "⭐ ${smartphone.rating}"

            // Set processor
            val processorView = cardView.findViewById<TextView>(R.id.tv_apple_processor)
            processorView.text = smartphone.processor

            // Set storage
            val storageView = cardView.findViewById<TextView>(R.id.tv_apple_storage)
            storageView.text = smartphone.storage.split("/")[0] // Take first storage option

            // Set camera
            val cameraView = cardView.findViewById<TextView>(R.id.tv_apple_camera)
            val cameraText = extractMainCamera(smartphone.camera)
            cameraView.text = cameraText

            // Set display
            val displayView = cardView.findViewById<TextView>(R.id.tv_apple_display)
            val displayText = extractDisplaySize(smartphone.display)
            displayView.text = displayText

            // Set description
            val descriptionView = cardView.findViewById<TextView>(R.id.tv_apple_description)
            descriptionView.text = smartphone.description

            // Set price
            val priceView = cardView.findViewById<TextView>(R.id.tv_apple_price)
            val formatter = NumberFormat.getInstance(Locale("id", "ID"))
            val formattedPrice = "Rp ${formatter.format(smartphone.price)}"
            priceView.text = formattedPrice

            // Set detail button click
            val detailButton = cardView.findViewById<TextView>(R.id.btn_apple_detail)
            detailButton.setOnClickListener {
                navigateToDetail(smartphone.id)
            }

        } catch (e: Exception) {
            Log.e("AppleFragment", "Error setting up ranking card", e)
        }
    ...
    private fun extractMainCamera(cameraString: String): String {
        // Extract main camera info from camera string
        return when {
            cameraString.contains("48MP") -> "48MP Pro"
            cameraString.contains("12MP") -> "12MP"
            else -> "48MP"
        }
    }
    ...
    private fun navigateToDetail(smartphoneId: String) {
        try {
            ...
            Log.d("AppleFragment", "Navigating to detail for smartphone: $smartphoneId")
            ...
            navController.navigate(R.id.action_apple_to_detail, bundle)

            Log.d("AppleFragment", "Navigation to detail initiated from Apple")

        } catch (e: Exception) {
            Log.e("AppleFragment", "Error navigating to detail", e)
        }
    }
}